...
saveRDS(best_model, file = "bestmodel.RDS")
bestmodel <- readRDS("~/GitHub/TFM-Statistical-Learning/bestmodel.RDS")

# Mejor modelo
print(bestmodel)

# Obtener los hiperparámetros óptimos del objeto de resultados
best_cp <- bestmodel$control$cp
best_maxdepth <- bestmodel$control$maxdepth
...
library(rpart.plot)
rpart.plot(final_tree, type = 3, box.palette = "auto", shadow.col = "gray")
...
predicted_labels <- predict(final_tree, test_data, type = "class")
